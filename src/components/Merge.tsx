...
export default function Merge(props: CardSynthesisPropsType) {
    ...
    useEffect(() => {
        if (props.isShow && web3React.account) {
            compoundCardConfig(props.CardInfo.cardLevel).then(res => {
                Contracts.example.toSBL(web3React.account as string, res.data.price).then((res: any) => {
                    let value = new BigNumber(res).div(10 ** 18).toString()
                    setMergePrice(parseFloat(value))
                })
            })
    ...
    function ApproveFun() {
        ...
        showLoding(true)
        Contracts.example.approve1(web3React.account as string, contractAddress.Merge, `${MergePrice}`).then(() => {
    ...
    return (
        <>
            {/* 确认合成 */}
            <Tips isShow={showEnterMerge} title={t('Expenses2')} subTitle={t('The evolving cost') + MergePrice + "SBL"} enterFun={mager} close={() => setShowEnterMerge(false)}></Tips>
                    ...
                    <div className="SynthesisItems">
                        ...
                        <div className="mergeCardItem">
                            ...
                            <div className="Decorate">
                                <div className="Price">{t('Expenses')}：</div><div className='Number'>{Math.floor(MergePrice)} SBL</div>
                            </div>
                            {
                                parseFloat(ApproveValue) > MergePrice ? <button onClick={() => { setShowEnterMerge(true) }}>{t('Confirm')}</button> : <button onClick={ApproveFun}>{t('Approve')}</button>
                            }
